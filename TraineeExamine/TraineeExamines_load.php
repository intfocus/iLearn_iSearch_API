<?php
   ...
   {
      if(strcmp($check_str, "searchTraineeExamines"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   //----- query -----
   //***Step16 页面搜索SQl语句 起始
   $str_query1 = "select te.TrainingId, te.UserId, te.RegisterDate, te.Status, ti.TrainingName, ti.TrainingMemo, ti.ApproreLevel, u.UserName, u.EmployeeId, ti.SpeakerName 
...
left join users as u on te.UserId = u.UserId where te.Status >=0 and ExamineUser like '%,$user_id,%' and te.Status <> ti.ApproreLevel";
   ...
   if($result = mysqli_query($link, $str_query1)){
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_TraineeExamine_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_TraineeExamine_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            $return_string = $return_string . "\" id=search_TraineeExamine_page_begin_no_" . ($i + 1) . " OnClick=clickSearchTraineeExaminesPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      //***Step7 function name ==> expandSearchTraineesContentFunc
      $return_string = $return_string . "</span>"
                       . "<span class=\"btn TrainingLogsexpandSR\" OnClick=\"expandSearchTraineeExaminesContentFunc();\">显示过长内容</span>"
                       . "</div>";                   
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_TraineeExamine_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_TraineeExamine_page_end_no_" . ($i + 1) . " OnClick=clickSearchTraineeExaminesPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                       . "<span class=\"btn TrainingLogsexpandSR\" OnClick=\"expandSearchTraineeExaminesContentFunc();\">显示过长内容</span>"
