<?php
   ...
   {
      if(strcmp($check_str, "searchDepts"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   if(($searchDeptsNameCode = check_name($_GET["searchDeptsNameCode"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   if(($searchDeptsfrom1 = check_range_begin($_GET["searchDeptsfrom1"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   }
   if(($searchDeptsto1 = check_range_end($_GET["searchDeptsto1"])) == SYMBOL_ERROR)
   {
      sleep(DELAY_SEC);
      echo SYMBOL_ERROR;
      return;
   ...
   $str_query1 = "
      select *
      from Depts where ParentId > 0 AND status";
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_dept_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_dept_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_dept_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_dept_page_begin_no_" . ($i + 1) . " OnClick=clickSearchDeptsPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_dept_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"DeptName fixWidth\">$DeptName</span></td>"
                  . "<td><span class=\"DeptCode fixWidth\">$DeptCode</span></td>"
                  . "<td>$StatusStr</td>"
                  . "<td>$EditTime</td>"
                  . "<td><A OnClick=\"actionSearchDepts($DeptId,$Status);\">$StatusAction</A><br/>"
                  . "<A OnClick=\"modifySearchDepts($DeptId);\">修改</A><br/>"
                  . "<A OnClick=\"deleteSearchDepts($DeptId);\">删除</A></td>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_dept_page_end_no_" . ($i + 1) . " OnClick=clickSearchDeptsPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
