...
////////////////////////////////////////
   ...
   {
      if(strcmp($check_str, "searchPrivileges"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   //TODO: trim space
   if (strlen($searchPrivilegesNameEmail) > 0)
   {
   ...
   if ($searchPrivilegesfrom1 != '')
      $str_query1 = $str_query1 . "AND U.EditTime >= '$searchPrivilegesfrom1' ";
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_privilege_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_privilege_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_privilege_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_privilege_page_begin_no_" . ($i + 1) . " OnClick=clickSearchPrivilegesPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_privilege_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td>$EditTime</td>"
                  . "<td>"
                  . "<A OnClick=\"modifySearchPrivileges($UserId);\">修改</A><br/>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_privilege_page_end_no_" . ($i + 1) . " OnClick=clickSearchPrivilegesPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
