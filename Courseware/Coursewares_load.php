<?php
   ...
   {
      if(strcmp($check_str, "searchCoursewares"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   //***Step16 页面搜索SQl语句 起始               
   $str_query1 = "
      select F.CoursewareId as CoursewareId, F.CoursewareName as CoursewareName, F.CoursewareDesc as CoursewareDesc, 
             F.Status as Status, F.EditTime as EditTime
      from Coursewares F where F.status";
   ...
   //TODO: trim space
   if (strlen($searchCoursewaresNameDesc) > 0)
   {
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_Courseware_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_Courseware_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_Courseware_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_Courseware_page_begin_no_" . ($i + 1) . " OnClick=clickSearchCoursewaresPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_Courseware_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"CoursewareName fixWidth\">$CoursewareName</span></td>"
                  . "<td><span class=\"CoursewareDesc fixWidth\">$CoursewareDesc</span></td>"
                  . "<td>$StatusStr</td>"
                  . "<td>$EditTime</td>"
                  . "<td><A OnClick=\"actionSearchCoursewares($CoursewareId,$Status);\">$StatusAction</A><br/>"
                  . "<A OnClick=\"modifySearchCoursewares($CoursewareId);\">修改</A><br/>"
                  . "<A OnClick=\"deleteSearchCoursewares($CoursewareId);\">删除</A></td>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_Courseware_page_end_no_" . ($i + 1) . " OnClick=clickSearchCoursewaresPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
