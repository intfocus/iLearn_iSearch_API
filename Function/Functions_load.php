<?php
   ...
   {
      if(strcmp($check_str, "searchFunctions"))
      {
         return SYMBOL_ERROR;
      }
      return $check_str;
   ...
   {
      $str_query1 = $str_query1 . "=2 ";
   }
   else if ($statusCheckbox == 3)
   {
      $str_query1 = $str_query1 . "=3 ";
   }
   else
   {
      $str_query1 = $str_query1 . ">=1 ";
   ...
   //TODO: trim space
   if (strlen($searchFunctionsName) > 0)
   {
      $str_query1 = $str_query1 . "AND (FunctionName like '%$searchFunctionsName%') ";
   }
   
   if ($searchFunctionsfrom17 != '')
      $str_query1 = $str_query1 . "AND CreatedTime >= '$searchFunctionsfrom17' ";
   if ($searchFunctionsto17 != '')
      $str_query1 = $str_query1 . "AND CreatedTime <= '$searchFunctionsto17' ";
   ...
   if($result = mysqli_query($link, $str_query1)){      
      ...
      $return_string = $return_string . "<div class=\"toolMenu\">"
                                      ...
                                      . "<input type=\"hidden\" name=search_Function_page_no value=1>"
                                      . "<input type=\"hidden\" name=search_Function_page_size value=" . $page_size . ">";
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            $return_string = $return_string . "<span class=\"search_Function_page";
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            //***Step6 function name ==> clickSearchNewsPage
            $return_string = $return_string . "\" id=search_Function_page_begin_no_" . ($i + 1) . " OnClick=clickSearchFunctionsPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      ...
      else
      {
         ...
         while ($i < $row_number)
         {
            if ($page_count == 0)
            {
               $return_string = $return_string . "<div id=\"search_Function_page" . $page_no . "\" ";
               if ($page_no == 1)
            ...
            if ($page_count < $page_size)
            {
               ...
               $return_string = $return_string 
                  ...
                  . "<td><span class=\"FunctionName fixWidth\">$FunctionName</span></td>"
                  . "<td><span class=\"FunctionCode fixWidth\">$FunctionType</span></td>"
                  . "<td>$CreatedTime</td>"
                  . "<td><A OnClick=\"modifySearchFunctions($FunctionId);\">修改</A><br/>"
                  . "<A OnClick=\"deleteSearchFunctions($FunctionId);\">删除</A></td>"
                  . "</tr>";
      ...
      if ($page_num > 1)
      {
         for ($i = 0; $i < $page_num; $i++)
         {
            ...
            if ($i + 1 == $page_default_no)
               $return_string = $return_string . " active";
            $return_string = $return_string . "\" id=search_Function_page_end_no_" . ($i + 1) . " OnClick=clickSearchFunctionsPage(this," . ($i + 1) . ");>" . ($i + 1) . "</span>";
         }
      }
      $return_string = $return_string . "</span>"
                                      . "</div>";
